...
class ItemHandlerTest {
    ...
    @Test
    fun getAllItems2() {
        val itemFlux = client.get().uri("/v2/items")
                .exchange()
                .expectStatus().isOk
                .expectHeader().contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    fun getItemByID() {
        client.get().uri("/v2/items/{id}", "ABC")
                .exchange()
                .expectStatus().isOk
                .expectBody()
    ...
    fun getItemByID_notFound() {
        client.get().uri("/v2/items/{id}", "DEF")
                .exchange()
    ...
    @Test
    fun deleteItem() {
        client.delete().uri("/v2/items/{id}", "ABC")
    ...
    @Test
    fun updateItem() {
        client.put().uri("/v2/items/{id}", "ABC")
                .accept(MediaType.APPLICATION_JSON)
    ...
    fun updateItem_notFound() {
        client.put().uri("/v2/items/{id}", "DEF")
                .accept(MediaType.APPLICATION_JSON)
                .body(Mono.just(Item(null, "Kindle", 85.0)), Item::class.java)
                .exchange()
                .expectStatus().isNotFound
    ...
    @Test
    fun error() {
        client.get().uri("/v2/error")
                .exchange()
                .expectStatus().is5xxServerError
                .expectBody()
                .jsonPath("$.message").isEqualTo("Exception occurred.")
    }
}
